...
class CardApplicationBasicInfoTest(unit_init.Base):
    ...
    def test_login_custom_name_english(self):
        self.basic_info_page("ff", "512236197807102659", "15765484677", CardUrl.CARD_APPLICATION_BASIC_INFO_URL)
    ...
    def test_login_custom_name_one(self):
        self.basic_info_page("你", "512236197807102659", "15765484677", CardUrl.CARD_APPLICATION_BASIC_INFO_URL)
        base_info_label = self.driver.find_element_by_xpath('/html/body/div[11]')
        self.assertEqual(base_info_label.text, '姓名请输入中文，请输入2-8个汉字')

    # 测试用户姓名为数字
    def test_login_custom_name_number(self):
        self.basic_info_page("111", "512236197807102659", "15765484677", CardUrl.CARD_APPLICATION_BASIC_INFO_URL)
        base_info_label = self.driver.find_element_by_xpath('/html/body/div[11]')
        self.assertEqual(base_info_label.text, '姓名请输入中文，请输入2-8个汉字')

    # 测试用户姓名为null
    def test_login_custom_name_null(self):
        self.basic_info_page("", "512236197807102659", "15765484677", CardUrl.CARD_APPLICATION_BASIC_INFO_URL)
        base_info_label = self.driver.find_element_by_xpath('/html/body/div[11]')
        self.assertEqual(base_info_label.text, '姓名不能为空')

    # 测试用户身份证少一位
    def test_login_custom_identity_miss(self):
        self.basic_info_page("你好", "51223619780710265", "15765484677", CardUrl.CARD_APPLICATION_BASIC_INFO_URL)
        base_info_label = self.driver.find_element_by_xpath('/html/body/div[11]')
        self.assertEqual(base_info_label.text, '请输入正确的18位证件号码')

    # 测试用户身份证有一位是英文
    def test_login_custom_identity_english(self):
        self.basic_info_page("你好", "51223619780710265m", "15765484677", CardUrl.CARD_APPLICATION_BASIC_INFO_URL)
        base_info_label = self.driver.find_element_by_xpath('/html/body/div[11]')
        self.assertEqual(base_info_label.text, '请输入正确的18位证件号码')

    # 测试用户身份证没填
    def test_login_custom_identity_null(self):
        self.basic_info_page("你好", "", "15765484677", CardUrl.CARD_APPLICATION_BASIC_INFO_URL)
        base_info_label = self.driver.find_element_by_xpath('/html/body/div[11]')
        self.assertEqual(base_info_label.text, '证件号码不能为空')

    # 测试用户身份证不合法
    def test_login_custom_identity_no(self):
        self.basic_info_page("你好", "512236197807102650", "15765484677", CardUrl.CARD_APPLICATION_BASIC_INFO_URL)
        base_info_label = self.driver.find_element_by_xpath('/html/body/div[11]')
        self.assertEqual(base_info_label.text, '请输入正确的18位证件号码')
