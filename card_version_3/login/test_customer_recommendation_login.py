...
class CustomerRecommendationLoginTest(unit_init.Base):
    ...
    def test_login_org_normal(self):
        self.login("李芽", Phone.create_phone(), CardUrl.CUSTOMER_RECO_LOGIN_URL, '1001')
        sleep(3)
    ...
    def test_login_null_name(self):
        self.login("", Phone.create_phone(), CardUrl.CUSTOMER_RECO_LOGIN_URL, '1001')
        sleep(3)
    ...
    def test_login_english_name(self):
        self.login("jingj", Phone.create_phone(), CardUrl.CUSTOMER_RECO_LOGIN_URL, '1001')
        sleep(2)
        self.assertIn("姓名为空或格式不正确", self.get_sms_error_text())
